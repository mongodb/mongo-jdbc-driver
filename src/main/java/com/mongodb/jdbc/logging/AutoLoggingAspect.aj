...
public aspect AutoLoggingAspect perthis(execution(com.mongodb.jdbc.*.new(..)))
{
    private MongoLogger logger = null;

    private int callLevel = 1;

    pointcut setLogger(MongoLogger logger): set(MongoLogger *.*) && args(logger) && !within(AutoLoggingAspect) ;
    ...
    after () throwing (Exception e) : execution(* *.*(..)) && !within(AutoLoggingAspect)
    {
        if (null != logger) {
            logger.log(
                    Level.SEVERE,
